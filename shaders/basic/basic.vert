...
vec2 fetchCPDef(int i) {
    vec2 v = texelFetch(pathDefCPBuf, i).xy;
    return v;
}

// Bezier evaluation (Bernstein basis)
...
// Catmull-Rom spline (uniform, tension=0.5) evaluation
// pathNumCP must be >= 2
void catmullRomParams(float t, out int p0, out int p1, out int p2, out int p3, out float lt) {
    ...
    p0 = max(0, p1 - 1);
    p2 = min(pathNumCP - 1, p1 + 1);
    p3 = min(pathNumCP - 1, p2 + 1);
    lt = segf - float(seg);
}
...
vec2 splinePoint(float t, bool defCurve) {
    ...
    if (pathNumCP == 2) {
        vec2 a = defCurve? fetchCPDef(0): fetchCPOrig(0);
        vec2 b = defCurve? fetchCPDef(1): fetchCPOrig(1);
        return mix(a, b, t);
    }
    int p0,p1,p2,p3; float lt;
    catmullRomParams(t, p0,p1,p2,p3, lt);

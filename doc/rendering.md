...
This document captures the rendering pipeline that is already implemented in the repository as of 2026-02.  
...
- Multiple queues are used because ordering requirements differ by stage (task lifecycle order, draw ordering, deferred command transport).

```mermaid
graph TD
    A["Puppet.update()"] --> B["registerRenderTasks()"]
    B -->|DFS registration| C["TaskScheduler queue"]
    C -->|fixed TaskOrder execution| D["executeRange()"]
    D --> E["Node runRender* handlers"]
    E --> F["RenderGraphBuilder item queue"]
    F -->|zSort desc then stable FIFO| G["RenderGraphBuilder.playback()"]
    G --> H["RenderCommandEmitter callbacks"]
    H --> I["OpenGL path immediate backend calls"]
    I --> J["OpenGL backend and GPU API"]
    H --> K["Queue emitter command queue"]
    K --> L["Deferred consumer (Unity etc.)"]

    subgraph LEG["Legend"]
        direction LR
        LS["Procedure/method"] -->|order| LQ["Queue"]
    end

    classDef queueNode fill:#FFE8A3,stroke:#A36A00,stroke-width:2px,color:#222;
    class C,F,K,LQ queueNode;
```
...
- `Puppet.update()` orchestrates:
  ...
  4. Call `RenderGraphBuilder.beginFrame()`
...
5. **Draw playback phase** -> `Puppet.draw()` calls `RenderGraphBuilder.playback(commandEmitter)`, which invokes part/mask/dynamic-composite emitter operations.
...
```mermaid
sequenceDiagram
    participant Node as "Node task handler"
    participant Graph as "RenderGraphBuilder"
    participant Pass as "RenderPass stack"
    participant Sort as "zSort sorter"
    participant Emitter as "RenderCommandEmitter / backend path"
    participant Puppet as "Puppet instance"
    Node->>Graph: pushDynamicComposite(...)
    Graph->>Pass: open DynamicComposite pass
    Pass->>Pass: accumulate child RenderItems
    Node->>Graph: popDynamicComposite(...)
    Graph->>Pass: finalize DynamicComposite scope
    Pass->>Sort: finalize*Pass →flatten child commands
    Sort->>Pass: merge into parent pass (zSort desc + sequence)
    Puppet->>Graph: playback(emitter)
    Graph->>Emitter: emit RenderCommandKind operations
```
...
3. **RenderGraph playback -> Emitter/Backend**  
   - In `Puppet.draw()`, `RenderGraphBuilder.playback(commandEmitter)` emits the final render operations.
...
sequenceDiagram
    ...
    participant Emitter as "RenderCommandEmitter"
    participant Backend
    Tree->>TaskQ: registerRenderTasks (DFS / zSort)
    TaskQ->>Node: executeRange(...) invokes runRenderBeginTask(ctx)
    Node->>RenderGraph: push scope / set hints
    TaskQ->>Node: executeRange(...) invokes runRenderTask(ctx)
    Node->>RenderGraph: enqueue commands
    TaskQ->>Node: executeRange(...) invokes runRenderEndTask(ctx)
    Node->>RenderGraph: pop scope / finalize
    RenderGraph->>Stack: RenderItems kept per scope
    Tree->>RenderGraph: Puppet.draw() calls playback(emitter)
    RenderGraph->>Emitter: invokes draw/mask/dynamic callbacks
    Emitter->>Backend: forwards RenderCommandKind operations
...
5. **Playback & GPU Calls** -> `Puppet.draw()` runs `RenderGraphBuilder.playback(commandEmitter)` and the emitter performs backend calls (or records commands).
...
```mermaid
sequenceDiagram
    participant Graph as RenderGraphBuilder playback
    participant Emitter as CommandQueueEmitter
    participant Queue as QueuedCommand[]
    Graph->>Emitter: applyMask(maskDrawable, isDodge)
...
sequenceDiagram
    ...
    Puppet->>Emitter: beginFrame(backend, gpuState)
    ...
    Puppet->>Emitter: endFrame(backend, gpuState)
```
...
### 6.5 Backend-specific execution differences (OpenGL vs Queue)

Common entry path:

- `Puppet` calls `RenderGraphBuilder.playback(emitter)`.
- `RenderGraphBuilder` invokes emitter callbacks (`drawPart`, `beginMask`, `applyMask`, `beginDynamicComposite`, ...).
- The difference starts at emitter implementation.

| Aspect | OpenGL backend (`backends/opengl/queue.d`) | Queue backend (`backends/queue/package.d`) |
|---|---|---|
| Emitter class | `RenderQueue` | `CommandQueueEmitter` |
| Command handling | Immediate dispatch to `RenderBackend` | Record `QueuedCommand[]` |
| Draw path | `drawPart` -> `activeBackend.drawPartPacket(packet)` | `drawPart` -> `record(RenderCommandKind.DrawPart, ...)` |
| Mask path | `beginMask/applyMask/...` directly call backend methods | `beginMask/applyMask/...` record `BeginMask/ApplyMask/...` |
| DynamicComposite path | Direct `beginDynamicComposite/endDynamicComposite` backend calls | Record `BeginDynamicComposite/EndDynamicComposite` |
| Shared buffers | Uploaded in `beginFrame` via backend upload calls | No GPU upload in emitter; queue records commands only |
| Output artifact | GPU side effects happen during playback | In-memory command queue consumed later by integration/runtime |
...
sequenceDiagram
    participant Puppet as Puppet instance
    participant Graph as RenderGraphBuilder
    participant Emitter as OpenGL RenderQueue
    participant Backend as OpenGL RenderingBackend
    participant GL as OpenGL API
    Puppet->>Graph: playback(emitter)
    Graph->>Emitter: draw/mask/dynamic callbacks
    Emitter->>Backend: direct backend method calls
    Backend->>GL: immediate GPU API calls
```
...
sequenceDiagram
    ...
    participant Graph as RenderGraphBuilder
    participant Emitter as CommandQueueEmitter
    participant CmdQ as QueuedCommand[]
    participant Consumer as Unity/integration consumer
    Puppet->>Graph: playback(emitter)
    Graph->>Emitter: draw/mask/dynamic callbacks
    Emitter->>CmdQ: record RenderCommandKind + payload
    Consumer->>CmdQ: read queuedCommands()
```
...
- `RenderGraphBuilder.beginFrame()` now occurs every update; nodes always enqueue their builders for the current frame.
- No cached command buffer exists. The emitter consumes builders immediately during `RenderGraphBuilder.playback(commandEmitter)`.
...
5. Invoke `RenderGraphBuilder.playback(commandEmitter)` during `Puppet.draw()`, letting the backend-specific emitter translate node references into GPU work.
...
- `RenderGraphBuilder.playback(commandEmitter)` immediately replays the builders when `Puppet.draw()` runs; no cached buffer is stored.
